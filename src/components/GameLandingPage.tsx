...
export const GameLandingPage: React.FC<GameLandingPageProps> = ({
  ...
  return (
    <div id="game-seo-landing-page" className="min-h-screen pb-20 font-['Plus_Jakarta_Sans',sans-serif]">
      {/* Top Breadcrumbs & Back Navigation Bar */}
      <div className="bg-slate-900/90 border-b border-slate-800 sticky top-16 sm:top-20 z-30 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
          
          <button
            id="landing-back-to-home-btn"
            onClick={onBackToHome}
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white text-xs sm:text-sm font-bold transition-all border border-slate-700 cursor-pointer shadow-sm active:scale-95"
          >
          ...
          <nav aria-label="Breadcrumb" className="hidden sm:flex items-center gap-1.5 text-xs text-slate-400">
            <button onClick={onBackToHome} className="hover:text-amber-400 transition-colors">Home</button>
            <span>/</span>
            <button onClick={onBackToHome} className="hover:text-amber-400 transition-colors">All Yono Games</button>
            <span>/</span>
            <span className="text-amber-400 font-bold truncate max-w-[180px]">{app.name} APK</span>
          </nav>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-amber-300 border border-slate-700 text-xs font-semibold flex items-center gap-1 transition-all cursor-pointer"
              title="Copy Page URL"
            >
              {copiedPageUrl ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
              <span className="hidden md:inline">{copiedPageUrl ? 'Link Copied!' : 'Copy Link'}</span>
            </button>
            <button
              onClick={handleShareWhatsApp}
              className="p-2 rounded-xl bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-300 border border-emerald-500/40 text-xs font-bold transition-all cursor-pointer"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8 space-y-8">
        ...
        <section id="game-hero-card" className="rounded-3xl bg-gradient-to-b from-slate-900/95 via-slate-900/90 to-slate-950 border border-amber-500/30 p-5 sm:p-8 shadow-2xl shadow-black/50 relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 sm:gap-8 relative z-10">
            ...
            <div className="flex-1 space-y-4">
              ...
              {/* Trust Pill */}
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-xs font-bold shadow-sm">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h1 className="text-2xl sm:text-4xl font-black text-white font-['Outfit',sans-serif] tracking-tight leading-tight">
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs sm:text-sm text-slate-300 pt-1">
                <div className="flex items-center gap-1.5 bg-amber-500/15 border border-amber-500/30 px-3 py-1 rounded-xl text-amber-300 font-extrabold">
                  <Star className="w-4 h-4 fill-amber-400 text-amber-400" />
                  <span>{app.rating} / 5.0</span>
                  <span className="text-[11px] text-amber-300/80 font-normal">({app.reviewsCount.toLocaleString()} Votes)</span>
                </div>

                <div className="flex items-center gap-1.5 bg-slate-800/90 border border-slate-700/80 px-3 py-1 rounded-xl font-medium">
                  <Download className="w-4 h-4 text-sky-400" />
                  <span>{app.downloads} Installs</span>
                </div>

                <div className="flex items-center gap-1.5 bg-slate-800/90 border border-slate-700/80 px-3 py-1 rounded-xl font-mono text-slate-300">
                  <span>Size: <strong>{app.apkSize}</strong></span>
                </div>

                <div className="flex items-center gap-1.5 bg-slate-800/90 border border-slate-700/80 px-3 py-1 rounded-xl text-emerald-400 font-semibold">
            ...
            <div className="w-full lg:w-96 rounded-2xl bg-slate-950/80 border border-slate-800 p-5 sm:p-6 space-y-4 shadow-xl">
              
              {/* Free Bonus Callout */}
              <div className="p-3.5 rounded-xl bg-gradient-to-r from-amber-500/20 via-amber-400/10 to-orange-500/20 border border-amber-500/40 flex items-center justify-between">
                <div>
                  <div className="text-[10px] text-amber-300 font-bold uppercase tracking-wider">Instant Sign-up Bonus</div>
                  <div className="text-xl sm:text-2xl font-black text-amber-400 font-['Outfit',sans-serif]">
                    ₹{app.signupBonus} {app.maxSignupBonus && <span className="text-sm font-semibold text-amber-300">- ₹{app.maxSignupBonus}</span>}
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-[10px] text-emerald-300 font-bold uppercase tracking-wider">Min UPI Cashout</div>
                  <div className="text-xl sm:text-2xl font-black text-emerald-400 font-['Outfit',sans-serif]">
                    ₹{app.minWithdrawal}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="text-[11px] text-slate-400 font-semibold flex items-center justify-between">
                  ...
                  <span className="text-emerald-400">Bonus Guaranteed</span>
                </div>
                <div 
                  onClick={handleCopyReferCode}
                  className="p-2.5 rounded-xl bg-slate-900 border border-slate-700/80 hover:border-amber-500/60 flex items-center justify-between cursor-pointer transition-colors group"
                >
                  <span className="font-mono font-black text-amber-300 text-sm tracking-wider">
                    {app.referCode}
                  </span>
                  <button className="flex items-center gap-1 text-xs font-bold text-slate-300 group-hover:text-white">
                    {copiedReferCode ? (
                      <>
              ...
              <div className="flex items-center justify-center gap-3 text-[11px] text-slate-400 font-medium pt-1">
                <span className="flex items-center gap-1">
                  <Zap className="w-3.5 h-3.5 text-amber-400" /> {app.withdrawalSpeed}
                </span>
                <span>•</span>
                <span className="flex items-center gap-1">
        ...
        <section id="game-specs-grid" className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-md flex items-center gap-3.5">
            ...
            <div>
              <div className="text-[11px] text-slate-400 uppercase font-semibold">Sign-Up Bonus</div>
              <div className="text-lg font-black text-amber-400">₹{app.signupBonus} Free</div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-md flex items-center gap-3.5">
            ...
            <div>
              <div className="text-[11px] text-slate-400 uppercase font-semibold">Min Withdrawal</div>
              <div className="text-lg font-black text-emerald-400">₹{app.minWithdrawal} (UPI)</div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-md flex items-center gap-3.5">
            ...
            <div>
              <div className="text-[11px] text-slate-400 uppercase font-semibold">Payout Speed</div>
              <div className="text-lg font-black text-sky-400">{app.withdrawalSpeed}</div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-md flex items-center gap-3.5">
            ...
            <div>
              <div className="text-[11px] text-slate-400 uppercase font-semibold">Referral Bonus</div>
              <div className="text-lg font-black text-purple-400">{app.referCommission || '30% Comm'}</div>
            </div>
          </div>
        </section>
        ...
        <section id="game-features-section" className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 rounded-3xl bg-slate-900/90 border border-slate-800 p-5 sm:p-6 space-y-4 shadow-lg">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {app.gamesList.map((g) => (
                <span
                  key={g}
                  className="px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-700/80 text-slate-200 text-xs font-semibold hover:border-amber-400 transition-colors"
                >
                  🎮 {g}
                </span>
              ))}
          ...
          <div className="lg:col-span-2 rounded-3xl bg-slate-900/90 border border-slate-800 p-5 sm:p-6 space-y-4 shadow-lg">
            <div className="flex items-center gap-2 pb-3 border-b border-slate-800">
            ...
            <div className="space-y-2.5">
              {app.features.map((feature, idx) => (
                <div key={idx} className="flex items-start gap-2.5 p-2.5 rounded-xl bg-slate-950/60 border border-slate-800/80 text-xs sm:text-sm text-slate-200">
        ...
        <section id="game-how-to-guide" className="rounded-3xl bg-slate-900/90 border border-slate-800 p-5 sm:p-8 space-y-6 shadow-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2 relative">
              ...
              <h3 className="text-sm font-bold text-white">Click Download APK</h3>
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2 relative">
              ...
              <h3 className="text-sm font-bold text-white">Allow Unknown Apps</h3>
              <p className="text-xs text-slate-400 leading-relaxed">
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2 relative">
              ...
              <h3 className="text-sm font-bold text-white">Register with Mobile OTP</h3>
              <p className="text-xs text-slate-400 leading-relaxed">
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2 relative">
              ...
              <h3 className="text-sm font-bold text-white">Play & Instant Withdraw</h3>
              <p className="text-xs text-slate-400 leading-relaxed">
        ...
        <section id="game-faq-section" className="rounded-3xl bg-slate-900/90 border border-slate-800 p-5 sm:p-8 space-y-6 shadow-xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Everything you need to know about bonuses, downloads & withdrawals</p>
            </div>
          </div>
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="w-full p-4 text-left flex items-center justify-between gap-4 text-sm font-bold text-slate-100 hover:text-amber-300 transition-colors"
                  >
                    <span>{faq.q}</span>
          ...
          <section id="related-apps-section" className="space-y-4">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-slate-400">Players who downloaded {app.name} also loved these:</p>
              </div>
              <button
                onClick={onBackToHome}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {relatedApps.map((relApp) => (
                <div
                  key={relApp.id}
                  className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 hover:border-amber-500/50 transition-all flex flex-col justify-between space-y-3 group shadow-md"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1 text-xs text-amber-400 font-bold mt-0.5">
                        ...
                        <span className="text-[10px] text-slate-400 font-normal">({relApp.downloads})</span>
                      </div>
                    </div>
                  </div>

                  <div className="p-2 rounded-xl bg-slate-950 border border-slate-800/80 flex items-center justify-between text-xs">
                    <span className="text-slate-400 text-[11px]">Free Bonus</span>
                    <span className="text-amber-400 font-extrabold">₹{relApp.signupBonus}</span>
                  </div>

                  <div className="flex items-center gap-2 pt-1">
                    <button
                      onClick={() => onSelectApp(relApp)}
                      className="flex-1 py-2 px-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold transition-all text-center cursor-pointer"
                    >
                      Read Details
                    </button>
                    <button
