        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3">
          {steps.map((step, idx) => {
            ...
            return (
              <div
                key={step.num}
                className="relative p-2.5 sm:p-3 rounded-xl bg-slate-950/70 border border-slate-800/90 flex items-center gap-2.5 hover:border-slate-700 transition-colors"
              >
                {/* Step Number + Icon */}
                ...
                {idx < steps.length - 1 && (
                  <ArrowRight className="w-3.5 h-3.5 text-slate-700 hidden md:block absolute -right-2 top-1/2 -translate-y-1/2 z-10" />
                )}
              </div>
            );
          })}
